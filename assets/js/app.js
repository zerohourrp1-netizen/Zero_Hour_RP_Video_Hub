...
async function init(){
  ...
  const cleanSupabaseUrl = String(cfg.supabaseUrl || '')
    .trim()
    .replace(/\/+$/, '')
    .replace(/\/rest\/v1$/i, '')
    .replace(/\/auth\/v1$/i, '');
  S.client = supabase.createClient(cleanSupabaseUrl, String(cfg.supabaseAnonKey || '').trim());
...
async function login(e){
  ...
  if (canUpload()) {
    ...
    toast('Signed in, but this account is not approved as owner, admin, or uploader.');
  }
}
